...
export default function Reports() {
  ...
  const formatYAxis = (val: number) => {
      if (val >= 1000000000) return `Rp ${Math.round(val / 1000000000)}B`;
      if (val >= 1000000) return `Rp ${Math.round(val / 1000000)}M`;
      return `Rp ${val}`;
  }

  const formatTooltip = (val: number) => {
    return [new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(val), 'Valuation'];
  }
