import { Eye, Edit2, Calendar, Filter, ChevronLeft, ChevronRight, Search, Trash2 } from 'lucide-react';
...
export default function Inventory() {
  ...
  const filteredAssets = useMemo(() => {
    return assets.filter(asset => {
      ...
      const matchesCat = catFilter ? asset.category === catFilter : true;
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left border-collapse">
            <thead className="bg-surface-container-low border-b border-outline-variant sticky top-0 z-10">
              <tr>
                <th className="py-3 px-4 text-xs font-semibold text-on-surface-variant uppercase whitespace-nowrap tracking-wider">Asset ID</th>
                <th className="py-3 px-4 text-xs font-semibold text-on-surface-variant uppercase whitespace-nowrap tracking-wider">Asset Name</th>
                <th className="py-3 px-4 text-xs font-semibold text-on-surface-variant uppercase whitespace-nowrap tracking-wider">Subsidiary</th>
                ...
                <th className="py-3 px-4 text-xs font-semibold text-on-surface-variant uppercase whitespace-nowrap tracking-wider">Purchase Date</th>
                <th className="py-3 px-4 text-xs font-semibold text-on-surface-variant uppercase whitespace-nowrap text-right tracking-wider">Valuation (IDR)</th>
                <th className="py-3 px-4 text-xs font-semibold text-on-surface-variant uppercase whitespace-nowrap text-center tracking-wider">Condition</th>
              ...
              {paginatedAssets.length === 0 ? (
                ...
                paginatedAssets.map((asset) => (
                  <tr key={asset.id} className="hover:bg-surface-container-low/50 transition-colors group">
                    <td className="py-4 px-4 font-mono text-secondary text-xs">{asset.id}</td>
                    <td className="py-4 px-4 font-semibold text-on-surface">{asset.name}</td>
                    <td className="py-4 px-4 text-on-surface-variant">{asset.subsidiary}</td>
                    <td className="py-4 px-4 text-on-surface">{asset.category}</td>
                    <td className="py-4 px-4 text-on-surface-variant font-mono text-xs">{asset.date}</td>
                    <td className="py-4 px-4 font-mono text-xs text-right text-on-surface">{formatIDR(asset.val)}</td>
                    <td className="py-4 px-4 text-center">
                      <span className="inline-flex items-center gap-1.5 px-2 py-1 bg-surface-container text-on-surface text-xs font-medium rounded-full border border-outline-variant/30">
                        <div className={cn(
                          "w-1.5 h-1.5 rounded-full",
                          asset.conditionLevel === 'good' ? "bg-emerald-500" : 
                          asset.conditionLevel === 'warning' ? "bg-amber-500" : "bg-error"
                        )} />
                        {asset.condition}
                      </span>
                    </td>
