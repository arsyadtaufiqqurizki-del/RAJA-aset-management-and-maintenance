    ...
    try {
      ...
      const response = await fetch('/api/maintenance', {
        ...
        body: JSON.stringify({
          ...
          cost: cost ? `Rp ${parseInt(cost.replace(/\D/g, ''), 10).toLocaleString('id-ID')}` : '--',
          estimatedCost: cost ? `Rp ${parseInt(cost.replace(/\D/g, ''), 10).toLocaleString('id-ID')}` : '--',
          actualCost: status === 'Completed' && actualCost ? `Rp ${parseInt(actualCost.replace(/\D/g, ''), 10).toLocaleString('id-ID')}` : '--',
          status,
        })
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-scrim/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl w-full max-w-lg shadow-xl overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-on-surface-variant mb-1">Estimated Cost (IDR)</label>
              <input 
                type="number"
            ...
            {status === 'Completed' && (
              <div>
                <label className="block text-sm font-medium text-on-surface-variant mb-1">Actual Cost (IDR)</label>
                <input 
                  type="number"
